...
FROM RENTAL, DETAILRENTAL, VIDEO, MOVIE 
WHERE RENTAL.RENT_NUM = DETAILRENTAL.RENT_NUM AND DETAILRENTAL.VID_NUM = VIDEO.VID_NUM AND VIDEO.MOVIE_NUM = MOVIE.MOVIE_NUM AND DETAIL_RETURNDATE <= DETAIL_DUEDATE;

22.
SELECT MEMBERSHIP.MEM_NUM, MEM_LNAME, MEM_FNAME, SUM(DETAILRENTAL.DETAIL_FEE) AS "Rental Fee Revenue" 
FROM MEMBERSHIP, RENTAL, DETAILRENTAL WHERE MEMBERSHIP.MEM_NUM = RENTAL.MEM_NUM AND RENTAL.RENT_NUM = DETAILRENTAL.RENT_NUM GROUP BY MEMBERSHIP.MEM_NUM, MEM_LNAME, MEM_FNAME;

23.
SELECT MOVIE_NUM, M.MOVIE_GENRE, AVGCOST AS "Average Cost", MOVIE_COST, (MOVIE_COST – AVGCOST)/AVGCOST * 100 AS "Percent Difference" 
FROM MOVIE M, (SELECT MOVIE_GENRE, AVG(MOVIE_COST) AS AVGCOST FROM MOVIE GROUP BY MOVIE_GENRE) S 
